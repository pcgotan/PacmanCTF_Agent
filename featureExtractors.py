...
class OffenseExtractor(FeatureExtractor):
    def getFeatures(self, state, action):
        ...
        disFromHome = self.getMazeDistance(
            (state.data.layout.width/2., myposition[1]), myPosition)
        ...
        # need to normalize
        feature['dis-from-home'] = float(disFromHome) / \
            (walls.width * walls.height)
        feature['dis-from-capsules'] = float(min([self.getMazeDistance(
            myPosition, dis) for dis in capsulePos])) / (walls.width * walls.height)
        ...
        for opponent in enemy:
            ...
            if pos:
                ...
                features["#-of-ghosts-1-step-away"] = sum(
                    (next_x, next_y) in Actions.getLegalNeighbors(pos, walls))
